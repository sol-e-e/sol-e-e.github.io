...
함수의 호출 방식에 따라 this에 바인딩 될 객체가 결정된다.
...
기존 함수에 new 연산자를 붙여서 호출하면 해당 함수는 생성자 함수로 동작한다.
 1. 빈 객체가 생성되면 함수 내의 this는 이 빈 객체를 가리킨다.
 2. this를 이용해 생성자 함수 내 프로퍼티와 메소드를 정의한다.
...
```
함수 내부의 this에 바인딩할 객체와 함수에 전달할 argument를 넘긴다. apply는 배열로 call은 나열해서 전달한다.
```javascript
...
```
bind는 this를 바꾼 새로운 함수를 생성하지만 함수 호출은 하지않는다. 한번만 동작해서 this의 변경이 불가하다.