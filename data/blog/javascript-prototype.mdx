...
프로토타입 기반 상속이란 객체가 다른 객체로부터 속성과 메서드를 직접 상속받는 방식입니다. 자바스크립트에서는 모든 객체가 생성될 때 내부 `[[Prototype]]` 슬롯을 통해 다른 객체(프로토타입)와 연결됩니다. 이 연결을 통해 객체는 자신이 직접 가지고 있지 않은 속성과 메서드를 프로토타입으로부터 상속받아 사용할 수 있습니다. \
...
console.log(rex.speak()) // "Rex howls loudly!" 인스턴스 속성 우선
delete rex.speak
console.log(rex.speak()) // "Rex barks!" 인스턴스 속성 삭제 후 프로토타입 속성 사용
delete Dog.prototype.speak
console.log(rex.speak()) // "Rex makes a noise." Dog 프로토타입 속성 삭제 후  Animal 프로토타입 속성 사용
```
